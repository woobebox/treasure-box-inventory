...
export interface Location extends EntityBase { parentId?: string | null; type: 'room' | 'cabinet' | 'drawer' | 'hook' | 'box' | 'other'; name: string; path: string; sortOrder: number; }
export interface Item extends EntityBase { createdBy: string; updatedBy: string; currentLocationId: string; coverPhotoId?: string | null; name: string; normalizedName: string; category: string; status: ItemStatus; notes?: string; dueAt?: string | null; }
