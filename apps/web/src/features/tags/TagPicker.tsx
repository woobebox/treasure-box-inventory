  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex gap-2">
        <input id="tag-input" value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); addTag(); } }} className="min-w-0 flex-1 rounded-xl border border-slate-300 px-3 py-2" placeholder="工具、冬季用品..." />
        <button type="button" onClick={addTag} className="rounded-xl bg-teal-700 px-3 py-2 text-sm font-semibold text-white">加入</button>
      </div>
