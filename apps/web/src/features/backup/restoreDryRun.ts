...
export function restoreDryRun(value: unknown): RestoreDryRunResult {
  ...
  if (manifest.history !== undefined && !Array.isArray(manifest.history)) errors.push('history 必須是陣列。');
  ...
  return { valid: errors.length === 0, errors, counts: { locations: manifest.locations?.length ?? 0, items: manifest.items?.length ?? 0, photos: manifest.photos?.length ?? 0, tags: manifest.tags?.length ?? 0, history: manifest.history?.length ?? 0 } };
}
